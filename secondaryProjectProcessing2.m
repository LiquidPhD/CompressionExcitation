...
clearvars;
baseFolder = 'D:\secondaryProject\020222\SW in individual samples\Stiff';
...
save([baseFolder,filesep,'Velocities.mat'],'Velocity')


% load([baseFolder,filesep,'Velocities.mat'])
...
for l = 1:size(topStack,3)
for k = 1:size(topStack,2,2)
    try
...
figure;
scatter(1:size(thicknessNotAvg,3),squeeze(thicknessNotAvg(:,382,:)*1000))
...
figure; scatter(topStrain,Velocity(2:end));
title('Velocity vs Strain %, Soft phantom')
ylabel('Velocity (m/s)')
xlabel('Strain (%)')