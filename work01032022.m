...
%% Pull data from Instron...
% CSVFiles = rdir('E:\GelatinHalfHalf20211221\gelation_phantom_RIP_20211221.is_comp_RawData\*.csv')
% load([baseFolder,filesep,'meanSensorInstron.mat'],'STRESS','STRAIN','STRESSm','STRAINm');
% 
% lastIndex = zeros(size(sensorMaskStack,2))
% for k = 1:size(sensorMaskStack,2)
%     lastIndex(:,k) = find(sensorSize(1:670,k,imageIterator),1,'last')
% end
sensorInstron = [46.92817
34.83434
31.917
34.94629
38.09789
...
% caxis(ax2,[0 1000]);
% caxis(ax2,[CLow CHigh])
  text(ax2,xCentroidsLeft,yCentroidsLeft,num2str(YMLeftAvg3),'FontSize',20, ...
        'HorizontalAlignment','center','BackgroundColor','black','Color','white','Margin',1);
    text(ax2,xCentroidsRight,yCentroidsRight,num2str(YMRightAvg3),'FontSize',20, ...
        'HorizontalAlignment','center','BackgroundColor','black','Color','white','Margin',1);
ax2.Visible = 'off';
linkprop([ax1 ax2],'Position');
% h = colorbar;
...
export_fig([baseFolder,filesep,'YMAvgOverlayInstron',num2str(imageIterator),'.png'])
save([baseFolder,filesep,'YMAvgOverlayInstron',num2str(imageIterator),'.fig'])
