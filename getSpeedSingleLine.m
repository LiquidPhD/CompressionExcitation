function [VelocityLeft,VelocityRight] = getSpeedSingleLine(displacement,Parameters,sideIndexSelector,newWorkingIndices)
        %% Data loading.
        % load backward; % [ADJUSTABLE]
        disp_z = displacement(:,:,newWorkingIndices);  % The first frame is normally very noisy.
...
%                 disp2 = abs(newDispRIGHT);
            ...
            for k = 1:size(disp2,3)
                spacetime(:,k) = mean(squeeze(disp2(yRegion-10:yRegion+10,xRegion,k)));
            end
            if roiScaled(1) <0
...
%                 export_fig([folders(folderIndex).folder,filesep,'DistanceVsTimeFitting.png'])
            ...
            uitable('Data',VelocityLeft')
                ...
                uitable('Data',[VelocityLeft',VelocityRight']);
            end
