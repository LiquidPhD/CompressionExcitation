...
blueInstron = [333.38753
    333.26121
    338.17255
    ]
blueInstron = mean(blueInstron)
redInstron = [1026.28445
    1077.58387
    1050.72453
    ]
...
clearvars;
close all force;
baseFolder = 'E:\Agar112921\AgarHalfHalf112921';
%
...
% Step 1: Get YM of sensor region from above and strain via calculation
...
LMS = modefilt(leftMaskStack(:,:,imageIterator),[51 51]);
vec_image_left = vec_image.*double(logical(LMS));
RMS = modefilt(rightMaskStack(:,:,imageIterator),[51 51]);
vec_image_right = vec_image.*double(logical(RMS));

% Step 3: Assuming stress is uniform axially, calc YM in left and right
% regions using YM = stress/strain
...
strain_left=strain_calculation2(corr_sample.*double(logical(leftMaskStack(:,:,imageIterator))),zaxis(2),1,100);
figure;
imagesc(abs(strain_left)/1000); colormap(hot); colorbar; caxis([0,0.5]);

strain_right=strain_calculation2(corr_sample.*double(logical(rightMaskStack(:,:,imageIterator))),zaxis(2),1,100);
figure;
imagesc(abs(strain_right)/1000); colormap(hot); colorbar; caxis([0,0.5]);

strain_sensor(isnan(strain_sensor)) = 0;
strain_whole=strain_sensor.*mask+strain_left.*double(logical(leftMaskStack(:,:,imageIterator)))+strain_right.*double(logical(rightMaskStack(:,:,imageIterator)));
figure;
imagesc(xaxis,zaxis,abs(strain_whole)/1000); colormap(hot); colorbar; caxis([0,0.5]);
% 
% unwrapped_left=phase_unwrap(vec_image_left);
% 
% for pos=1:size(vec_image_left,2)
%     surf_dis(pos)=nanmedian(unwrapped_left(sensor_bdl(pos)+5:sensor_bdl(pos)+15,pos));
%     corr_sample(:,pos)=(unwrapped_left(:,pos)-surf_dis(pos)).*double(logical(leftMaskStack(:,pos,imageIterator)));
% end


YMLeft = stressSensor./(abs(strain_left)/1000);
YMLeft(isinf(YMLeft)) = 0;
figure; imagesc(YMLeft/1000); caxis([0 1000]);
leftMask = logical(leftMaskStack(:,:,imageIterator));
YMLeftAvg = mean(rmoutliers(YMLeft(leftMask)));

YMRight = stressSensor./(abs(strain_right)/1000);
YMRight(isinf(YMRight)) = 0;
figure; imagesc(YMRight/1000); caxis([0 1000]);
rightMask = logical(rightMaskStack(:,:,imageIterator));
YMRightAvg = mean(rmoutliers(YMRight(rightMask)));
