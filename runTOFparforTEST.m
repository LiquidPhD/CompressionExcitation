function [TOF_speed] = runTOFparfor(particleVelocity,Parameters,folderIndex,position,N_radius)
debugMode = 0;
...
%         N_radius = 25; % Half distance for choosing reference waveform. [ADJUSTABLE]
        tic();
        kernelCounter = evalin('base','kernelCounter')
        if kernelCounter == 1 %|| ~exist('b','var')
            % for ii = 1:Nz_desamp
            figure;
