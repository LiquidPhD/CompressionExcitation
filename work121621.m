...
vec_image = Loupas_phase_shift(:,:,imageIterator);
...
unwrapped_sample=phase_unwrap(vec_image_sample).*mask2;
...
% unwrapped_sample = fitted_displacement;

for pos=1:size(vec_image_sample,2)
    surf_dis(pos)=nanmedian(unwrapped_sample(sensor_bdl(pos):sensor_bdl(pos),pos));
...
corrected_sample = unwrapped_sample-surf_dis2';
corrected_sample = corrected_sample.*mask2;
figure; imagesc(corrected_sample);
...
figure; imagesc(corrected_left); colormap(jet); figure; imagesc(corrected_right); colormap(jet)

x_kern = 20;
% dis_cumsum = dis_cumsum(:,:,end);
% dis_cumsum = dis_cumsum';
vec_image = corrected_sample';
BScanWidth = size(vec_image,1);
...
corrected_sample = fitted_displacement;
...
[~,y] = ginput(1)
YM_sensor = 3*speed_sensor.^2;
YMMean = mean(YM_sensor(round(y):end,:),1,'omitnan');
figure; plot(YMMean)
...
stressSensor = YMMean.*strain_sensor; stressSensor = mean(stressSensor,1,'omitnan');figure; plot(stressSensor)
...
figure; imagesc(YMLeft); caxis([0 100]);
...
YMWhole = YMWhole/1000;
figure; imagesc(YMWhole); colormap(jet); caxis([0 20])
...
YMLeft = abs(stressSensor2./(abs(strain_left)/1000));
YMLeft(isinf(YMLeft)) = NaN;

YMLeftAvg = mean(rmoutliers(YMLeft(goodLeft)),'omitnan')


YMRight = abs(stressSensor2./(abs(strain_right)/1000));
YMRight(isinf(YMRight)) = NaN;
% rightMask = logical(rightMaskStack(:,:,imageIterator));
YMRightAvg = mean(rmoutliers(YMRight(goodRight)),'omitnan');
YMLeftAvg = mean(rmoutliers(YMLeft(goodLeft)),'omitnan')
...
YMLeft(isnan(YMLeft)) = 0;
YMRight(isnan(YMRight)) = 0;
YMWhole = YMLeft + YMRight;
YMWhole = YMWhole;
figure; imagesc(YMWhole); caxis([0 100]); colormap(jet);
[~,y] = ginput(1);
close all force; 

figure; imagesc(xaxis,zaxis(1:round(y)),YMWhole(1:round(y),:)); caxis([0 200]); colormap(jet)
xlabel('Distance (mm)')
ylabel('Distance (mm)')
export_fig([baseFolder,filesep,'YMMap',num2str(imageIterator),'.png'])

figure; imagesc(xaxis,zaxis(1:round(y)),mat2gray(abs(IQData(1:round(y),:,imageIterator)))); caxis([0 0.1]); colormap(gray)
xlabel('Distance (mm)')
ylabel('Distance (mm)')


YMLeftMed = median(rmoutliers(YMLeft(goodLeft)),'omitnan')
YMRightMed = median(rmoutliers(YMRight(goodRight)),'omitnan')
% YMRightAvg = mean(rmoutliers(YMRight(goodRight)),'omitnan')
% YMLeftAvg = mean(rmoutliers(YMLeft(goodLeft)),'omitnan')

YMLeft = (YMLeft.*leftMaskStack(:,:,imageIterator))/1000;
YMRight = (YMRight.*rightMaskStack(:,:,imageIterator))/1000;

YMWhole = YMLeft + YMRight;
figure; imagesc(YMWhole); caxis([0 50]); colormap(jet)
